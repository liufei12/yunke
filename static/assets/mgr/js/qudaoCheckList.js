$(function(){
	...
	$("#table-role .role-edit").click(function(event){
		...
		$.post('/qudao.org.getOrgCheckList', {bid: bid}).done(function(data){
			data = jQuery.parseJSON(data);
			...
			if (data && data.data) {
				...
				$(el).find("#label-15").html(data.qudaoUserName + ' 来自 '+ data.qudaoUserFrom)
				$(el).find("#label-16").html(data.constract_num)
				$(el).find("#label-17").html(data.tax_rate)
				$(el).find("#label-18").html(data.separate_rate)
	...
	$("#window-role-update .btn-primary").click(function(event){
		...
		$.post('/qudao.org.updateOrgCheck', params).done(function(data){
			data =jQuery.parseJSON(data);
			if (data && data.code == 0) {
				$(el).modal('hide');
				window.location.reload();
