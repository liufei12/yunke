$(function(){
	...
	$("#window-node-create .btn-primary").click(function(event){
		...
		var params = {
			...
			'is_pro' : $(el).find("select[name=is_pro]").val(),
		};
		if (!params.node_title) {
			layer.msg('标题名称不能为空');
			return;
		}
		$.post('/platform/orgnode/addNode', params).done(function(data){
			data =jQuery.parseJSON(data); 
			if (data && data.result.code == 0) {
				$(el).modal('hide');
				window.location.reload();
	...
	$(".navigat_box .node-edit").click(function(event){
		...
		$.get('/platform/orgnode/getNode', {nid: nid}).done(function(data){
			data =jQuery.parseJSON(data);
			...
			if (data) {
				...
				$(el).find("select[name=is_pro]").val(data.is_pro)
				$(el).modal('show');
			} else {
				layer.msg('获取失败!');
			}
	...
	$("#window-node-update .btn-primary").click(function(event){
		...
		$.post('/platform/orgnode/updateNode', params).done(function(data){
			data =jQuery.parseJSON(data); 
			if (data && data.result && data.result.code == 0) {
				$(el).modal('hide');
				window.location.reload();
	...
	$(".navigat_box .node-open").click(function(event){
		...
		$.post('/platform/orgnode/setstatus', {nid: nid,flag:0 }).done(function(data){
			data =jQuery.parseJSON(data); 
			if (data && data.result && data.result.code == 0) {
				layer.msg('启用成功！');
				window.location.reload();	
	...
	$(".navigat_box .node-off").click(function(event){
		var nid = $(this).parent().find("input[name='nid']").val();
		$.post('/platform/orgnode/setstatus', {nid: nid,flag:1}).done(function(data){
			data =jQuery.parseJSON(data); 
			if (data && data.result && data.result.code == 0) {
				layer.msg('禁用成功！');
				window.location.reload();	
