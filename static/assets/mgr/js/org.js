$(function(){
	...
	$("#table-role .role-show").click(function(event){
		...
		$.post('/platform.org.getOrgShowInfo', {bid: bid}).done(function(data){
			...
			if (data && data.data) {
				...
				$(el).find('input[name=org_type]').each(function(){
					if($(this).val() == data.org_type){
						$(this).attr('checked',true);
					}
				});	
				if(data.option == 1){
					$(el).find(".pro_option").show();
				}else{
					$(el).find(".pro_option").hide();
				}
				if(data.is_pro == 1){
					$(el).find("#label-18").attr("checked","checked");
					
				}else{
					$(el).find("#label-19").attr("checked","checked");
				}
				var tr;var d;
		...
		$("input[name='sales_man']").keyup(function(){
			var sname=$.trim($(this).val());
				$.get("/qudao/org/getQudaoUserByUserName?userName="+sname,{  },function(r){
					if(r.result){
						var html='';
	...
	$("#window-role-show .btn-primary").click(function(event){
		...
		var params = {
			...
			'fk_qudao' : $(el).find("input[name=fk_qudao]").val(),
			'fk_qudao_user' : $(el).find("input[name=fk_qudao_user]").val(),
			'constract_num' : $(el).find("#label-12").val(),
			'tax_rate' : $(el).find("#label-13").val(),
			'separate_rate' : $(el).find("#label-14").val(),
			'option' : $(el).find("input[name='option']:checked").val(),
			'income_show' : $(el).find("input[name='income_show']:checked").val(),
			'org_type' : $(el).find("input[name='org_type']:checked").val(),
		};
